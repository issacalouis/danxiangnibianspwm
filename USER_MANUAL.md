...
当前控制假设：输出频率默认 `50Hz`，输出 LC 为 `1mH + 10uF`，控制周期为 `20kHz / 50us`。模型归一化母线参数使用 `XTQ3_VBUS_NORM = 18.0`。调试时必须从低压、限流开始。
...
| --- | --- | --- | --- | --- |
| 输出电压 `Vout` | ADC1_CH5 | PA5 | ADC7 | `(Vadc - v_bias) * 55.8` |
| 输出电流 `Iout` | ADC2_CH14 | PC4 | ADC2 | `(Vadc - iout_bias) * 2.828` |
| 输入电流 `Iin` | ADC3_CH10 | PC0 | ADC6 | `(Vadc - iin_bias) * 2.828` |

默认偏置为 `1.6900V`。在 `STOP` 状态下可用键盘按 `0.0001V` 步进微调三路偏置。
...
inverter_voltage_cmd = v_ref + KP * control_v_err + resonant - iout_damping * i_out;
M = clamp(inverter_voltage_cmd / 18.0, -modulation_limit, modulation_limit);
...
- ADC 控制采样一阶滤波 `ADC_FILTER_ALPHA = 0.05`，比之前更偏抗噪；死区误差补偿代码已保留，但默认补偿量为 `0.0`，等效关闭。
...
校准界面：`CAL V`、`CAL IO`、`CAL II` 分别调整电压、输出电流、输入电流采样偏置；`BIAS` 为当前偏置电压，`VAL` 为当前换算值。
...
1. 保持 `STOP`，确认 `V/Io/Ii` 零输入接近 0。
2. 若零点不准，按 `C` 进入校准界面，用 `1/2` 调整偏置。
...
| 现象 | 可能原因 | 处理建议 |
| --- | --- | --- |
| 零输入时 `V/Io/Ii` 不接近 0 | 采样偏置不准 | 在 `STOP` 下用校准界面调整偏置 |
...
| 相邻 PWM 周期占空比轻微跳动 | ADC 采到开关尖峰、QPR 对噪声过度反应或误差极限环 | 先确认 TIM1_CC2 触发点，再对比 ADC_FILTER_ALPHA、BOOST_QPR_KP 和误差死区 |
...
- QPR 谐振系数会跟随输出频率重算，调制量按固定归一化母线 `XTQ3_VBUS_NORM = 18V` 限幅。
