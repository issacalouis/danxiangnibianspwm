...
- ADC 换算后的瞬时值先做轻量一阶滤波 `ADC_FILTER_ALPHA = 0.25`，只用于压制开关采样噪声，闭环瞬时误差仍使用滤波后的 `v_out/i_out`。OLED 主界面显示的 `Vr/Ir` 为 RMS 值，使用滤波后的瞬时值平方后再由 `RMS_FILTER_ALPHA = 0.001` 做均值低通。`SV_active` 到达目标后，慢速 RMS 幅值外环会按 `SV_active - Vrms` 修正前馈幅值，默认 `KP=0.10`、`KI=4.0`、最大修正 `5Vrms`。死区误差补偿代码已保留，但默认补偿量为 `0.0`，等效关闭。
...
| `F` | 输出频率 |
| `LI` | 输出过流保护阈值 |
| `Vr` | 输出电压 RMS，来自滤波后的输出电压瞬时值 |
| `Ir` | 输出电流 RMS，来自滤波后的输出电流瞬时值 |
| `Ii` | 输入电流采样换算值，来自 PC0 |
| `M` | 最近一个输出周期内的调制峰值；RMS 幅值外环生效后，`M` 会随 `SV - Vr` 自动上调或下调 |

校准界面：`CAL V`、`CAL IO`、`CAL II` 分别调整电压、输出电流、输入电流采样偏置；`CAL VG`、`CAL IOG`、`CAL IIG` 分别调整三路采样比例。`BIAS` 为当前偏置电压，`GAIN` 为当前换算比例，`VAL` 为当前换算值。

## 7. 键盘操作

| 按键 | 功能 |
| --- | --- |
| `A` | 切换到电压目标编辑，OLED 显示 `SET V` |
| `B` | 切换到电流限制编辑，OLED 显示 `SET I` |
| `C` | 在 `STOP` 下循环 `CAL V / CAL VG / CAL IO / CAL IOG / CAL II / CAL IIG / NORMAL` |
| `D` | `STOP -> ARM -> RUN`，运行中或故障中再按回到 `STOP` |
| `1` | 普通界面增加当前编辑项；校准界面偏置 `+0.0001V` 或比例 `+0.1` |
| `2` | 普通界面减少当前编辑项；校准界面偏置 `-0.0001V` 或比例 `-0.1` |
...
2. 若零点不准，按 `C` 进入偏置校准界面，用 `1/2` 调整偏置；若比例不准，继续按 `C` 进入 `CAL VG/CAL IOG/CAL IIG`，用 `1/2` 按 `0.1` 步进调整比例。
...
| 实测电压/电流与显示成比例偏差 | 采样比例不准 | 在 `STOP` 下进入 `CAL VG/CAL IOG/CAL IIG` 调整比例 |
| 按 `D` 后停在 `ARM` | 启动自检未通过 | 检查零点、采样线和噪声 |
| `RUN` 后 `M` 长期接近上限 | 目标过高、母线不足、负载过重、采样极性错误或 RMS 幅值外环已打满 | 降低 `SV`，限流低压重新测 |
...
- RUN 状态下实际执行 QPR 电压环 + 输出电流阻尼 + SPWM，不再是双 PI 串级。
...
采样中断里先把 ADC 原始值换算成瞬时电压/电流，再滤波，再计算 RMS：
...
其中 `v_f/i_f` 是滤波后的瞬时量，`v_rms_sq/i_rms_sq` 是平方值的低通均值。这个算法是指数滑动 RMS，不是严格整周期窗口 RMS，显示会比瞬时值更稳。RMS 幅值外环也使用这个 `Vrms`，只在软启动到达目标附近后启用，用来消除输出幅值的稳态误差。

### 采样偏置与比例可调

- 三路采样使用运行时参数换算：`V = (Vadc - v_bias) * v_gain`，`Io = (Vadc - iout_bias) * iout_gain`，`Ii = (Vadc - iin_bias) * iin_gain`。
- 在 `STOP` 状态下按 `C` 循环偏置校准和比例校准；`CAL V/CAL IO/CAL II` 调偏置，`CAL VG/CAL IOG/CAL IIG` 调比例；每一路偏置界面后紧跟该路比例界面。
