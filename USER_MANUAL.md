...
- ADC_FILTER_ALPHA=0.25，电压/电流瞬时一阶滤波启用，用于 QPR/比例波形环、输出电流阻尼、整周期 RMS 显示、Irms 过流和 RMS 幅值外环，避免开关采样尖峰直接进入控制量。OLED 主界面显示的 Vr/Ir 为按输出相位整周期计算的 RMS 值；频率改变后会等待一个完整新周期再恢复 RMS 外环积分。慢速 RMS 幅值外环使用 SV_active - Vrms 修正前馈幅值：KP=0.06、KI=2.0、最大修正 4.0Vrms；用户 SV 上限仍为 26.0Vrms，内部前馈命令上限为 30.0Vrms。死区误差补偿默认启用，BOOST_DEADTIME_COMP_MODULATION=0.007。
...
1. 没有母线电压 ADC，调制仍按模型归一化 `36V` 处理；`SV` 设到 `26Vrms` 时会接近或进入满调制饱和。RMS 外环内部前馈命令最高可到 `30Vrms` 用于补偿高频段有效输出损失，但最终仍会被调制上限限制，不能保证无削顶。
2. 参数不会掉电保存。
...
- 载波频率保持 `20kHz` 不变；基于 `Io` 极性的死区误差前馈补偿默认启用，补偿量为 `0.007` 调制深度，用于改善 2A 带载下的有效输出损失。
